...
/** 2JS */
function DEUXJS_numtoletDate($datenum) {

	$newMonth = array(
		"01"=>"JAN", "02"=>"FEV", "03"=>"MAR", "04"=>"AVR", "05"=>"MAI",
		"06"=>"JUI", "07"=>"JUL", "08"=>"AOU", "09"=>"SEP", "10"=>"OCT",
		"11"=>"NOV", "12"=>"DEC");
		
	return substr($datenum, 6, 2) . ' ' . $newMonth[substr($datenum, 4, 2)] . ' ' . substr($datenum, 0, 4);
}


// Fonction enlevant les liens du post et retournant le texte
function removeImg($string) {
	
	$start = strpos($string, '<a ');
	$stop = strpos($string, '</a>');
	return substr($string, 0, $start) . substr($string, $stop-$start+4);
}

/** Fonction d'affichage de la premi�re image de galerie
	http://www.geekeries.fr/developpement-web/afficher-images-articles-wordpress-4173 */
//function thumb_gallerys($idpage, $size = '230x100') {
function thumb_gallerys($idpage, $pageIndex = FALSE) {
	//add_image_size( 'my-custom-size', 180, 250, true );
	...
	'orderby' => 'menu_order'
			...
			$atturl   = wp_get_attachment_url($image->ID); // Adresse de l'image en taille r�elle
			//$attlink  = get_attachment_link($image->ID); // Adresse de la page de l'image
			$postlink = get_permalink($idpage); // Adresse de l'article
			//$atttitle = apply_filters('the_title',$image->post_title); //  Le titre de l'article		
			
			echo '<a class="thumb" name="leaf" href="' . $postlink . '">
					' . ($pageIndex ? 
					'<img style="padding-bottom:5px;" 
						src="wp-content/plugins/imageresizer.php/1.jpg?width=500&height=100&cropratio=21:5&image=' . $atturl . '">' :
					'<img src="wp-content/plugins/imageresizer.php/1.jpg?width=230&height=300&cropratio=9:4&image=' . $atturl . '">') . 
					'</a>';
		}
}
/**
...
function list_gallerys($args = '') {
	...
	$pages = get_pages($r);
	// Trie de la plus r�cente � la plus ancienne galerie
	$pages = array_reverse($pages);
	$cptGal = 0;
	foreach ($pages as $page) {
	
		// V�rifie si le nombre max est atteint
		if ($r['nbGal'] > 0 && $cptGal == $r['nbGal']) {
			break;
		} else {
	
			echo '<div class="thumbImg">';
			
			if ($r['pageIndex'] === FALSE) {
				thumb_gallerys($page->ID);
				echo '</div>
					<div class="thumbText">
						<span class="thumbTitle">' . $page->post_title . '</span><br />' . 
						str_replace('[gallery]', '', $page->post_content) . 
						'<br /><a href="index.php?page_id=' . $page->ID . '" class="sidebarMenu">voir [+]</a>';
			} else {
			
				thumb_gallerys($page->ID, TRUE);
			}
			echo '</div>
				<div class="clear"></div>';
			if ($r['pageIndex'] === FALSE) {
...
function attachment_toolbox($size = thumbnail) {
	...
	'orderby' => 'menu_order'
		...
		foreach($images as $image) {
			...
			//echo '<a href="'.$atturl.'">'.$attimg.'</a>'; // Affiche les donn�es
			
			
			echo '<li>
					<a class="thumb" name="leaf" href="' . $atturl . '" title="' . $atttitle . '">
						' . $attimg . '
					</a>
					<div class="caption">
						<div class="image-title">' . $atttitle . '</div>
					</div>
				</li>';
...
/* La fonction wp_get_archive_advanced permet d�afficher les archives par mois ET par ann�e, limitant ainsi le nombre de lignes affich�es dans la sidebar. */
