...
<?php
				...
				foreach ($custom_fields as $field => $fieldDatas) {
					if ($field != '_edit_lock' && $field != '_edit_last' && $field != 'web') {
						foreach ($fieldDatas as $hour) {
							if (!isset($horaires[$field][$hour])) {
								$horaires[$field][$hour] = array();
							}
							array_push($horaires[$field][$hour], $post->ID);
						}
					} else if ($field == 'web') {
	...
	foreach ($jours as $jour) {
		...
		$flagProg = FALSE;
		// Pour chaque horaire
		foreach ($horaires[$jour] as $hour => $listIdProg) {
			
			// Flage la présence de programmes pour ce jour
			$flagProg = TRUE;
			
			// Passe les prog en revue
			foreach ($listIdProg as $idProg) {
				if ($cptLine == 1) {
					...
					echo '<div class="progLeft">
							<p>
								...
								(count($listIdProg) > 1 ? '&nbsp;(Toutes les quinzaines)' : '') . 
								'</span>
							</p>
							...
							<div class="clear"></div>' . 
									...
									'<br /><a href="index.php?page_id=212&idprog=' . $programmes[$idProg]['catId'] . '&idem=' . 
												...
												<a href="index.php?page_id=316&idprog=' . $programmes[$idProg]['catId'] . '">[PODCASTS]</a>
						</div>';
					$cptLine ++;
